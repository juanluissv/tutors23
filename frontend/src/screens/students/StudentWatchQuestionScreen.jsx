...
const PlayIconLarge = () => (
	...
	>
		...
		<path d='M19 15l14 9-14 9V15z' fill='url(#sq-play-lg)' />
		<defs>
			<linearGradient
				id='sq-play-lg'
				x1='19'
...
/**
 * Watch the student’s recorded question video (GET metadata + signed video URL).
 */
function StudentWatchQuestionScreen () {
	...
	const {
		...
		error,
		refetch,
	} = useGetStudentQuestionByIdQuery(questionId, {
		skip: !canFetch,
	})

	const videoSrc =
		question?.mediaId && String(question.mediaId).trim() !== ''
			? `${QUESTIONS_URL}/student/question/${questionId}/video`
			: null
	...
	useEffect(() => {
		if (!studentInfo) {
			const next = encodeURIComponent(
				questionId !== ''
					? `/students/watchquestion/${questionId}`
					: '/students/askteacher',
			)
			navigate(`/login?redirect=${next}`, { replace: true })
		}
	}, [studentInfo, navigate, questionId])
...
export default StudentWatchQuestionScreen
