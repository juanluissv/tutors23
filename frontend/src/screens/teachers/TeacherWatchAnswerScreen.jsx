...
const PlayIconLarge = () => (
	...
	>
		...
		<path d='M19 15l14 9-14 9V15z' fill='url(#twa-play-lg)' />
		<defs>
			<linearGradient
				id='twa-play-lg'
				x1='19'
...
function TeacherWatchAnswerScreen () {
	...
	const {
		...
		error,
		refetch,
	} = useGetTeacherAnswerByIdQuery(answerId, {
		skip: !canFetch,
	})
	...
	const answerHeadline = useMemo(() => {
		...
		if (answer?.question && typeof answer.question === 'object') {
			const qt = answer.question.title
			if (qt != null && String(qt).trim() !== '') {
				return String(qt).trim()
			}
		}
		return 'Tu respuesta'
	}, [answer])

	const questionIdForLinks = useMemo(() => {
		const q = answer?.question
		if (q != null && typeof q === 'object' && q._id != null) {
			return String(q._id)
		}
			...
			<div className='main-container'>
				...
				<div className='main-content'>
					...
					<div className='content-area'>
						<div className='watch-new watch-new--compact-player'>
							{/* <Link
							...
							{!canFetch && (
								<>
									<p className='watch-new__description'>
										...
										válido. Abre una respuesta desde
										preguntas anteriores.
									</p>
									<Link
										to='/teachers/oldquestions'
							...
							{canFetch && answer && (
								<>
										...
										<p className='watch-new__description'>
											{answer.description
												&& String(answer.description)
													.trim() !== ''
												? answer.description
												: 'Respondiste con texto '
													+ 'y/o video arriba.'}
										</p>
										<span className='watch-new__meta'>
											de{' '}
											{studentDisplayName(answer.student)}
											{formatAnswerDate(
												answer.dateCreated
													|| answer.createdAt,
											)
									...
									{/* {questionIdForLinks !== ''
										&& isLikelyMongoId(questionIdForLinks)
										? (
											<Link
												to={
...
export default TeacherWatchAnswerScreen
