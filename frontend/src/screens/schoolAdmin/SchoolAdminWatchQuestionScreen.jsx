...
const PlayIconLarge = () => (
	...
	>
		...
		<path d='M19 15l14 9-14 9V15z' fill='url(#saq-play-lg)' />
		<defs>
			<linearGradient
				id='saq-play-lg'
				x1='19'
...
function SchoolAdminWatchQuestionScreen () {
	const navigate = useNavigate()
	const { questionId: questionIdParam } = useParams()
	const questionId = questionIdParam ? String(questionIdParam).trim() : ''
	...
	const {
		...
		error,
		refetch,
	} = useGetSchoolAdminQuestionByIdQuery(questionId, {
		skip: !canFetch || !schoolAdminInfo,
	})

	const videoSrc =
		question?.mediaId && String(question.mediaId).trim() !== ''
			? `${QUESTIONS_URL}/schooladmin/question/${questionId}/video`
			: null
	...
	const hasTeacher = question?.teacher
		&& typeof question.teacher === 'object'
						...
						<div className='watch-new'>
							...
							{canFetch && question && (
								<>
									<h1
										className={
											`watch-new__subject watch-new__subject--${subjectColor}`
										}
									>
										{subjectTitle}
									</h1>
									<div
										className={
											`watch-new__subject-line watch-new__subject-line--${subjectColor}`
										}
									/>
...
export default SchoolAdminWatchQuestionScreen
